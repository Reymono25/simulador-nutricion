...
const THEME_MAP = {
  '1': 'TEMA I: ELABORACIÓN DE MENUS',
  '2': 'TEMA II: NUTRICIÓN CLÍNICA E INVESTIGACIÓN EN NUTRICIÓN',
  '3': 'TEMA III: NUTRICIÓN COMUNITARIA',
  '4': 'TEMA IV: GESTIÓN DE SERVICIOS DE ALIMENTACIÓN'
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Theme load
  const cachedTheme = localStorage.getItem('nutrisim-theme') || 'light';
  if (cachedTheme === 'dark') {
  ...
  // Sound load
  const cachedSound = localStorage.getItem('nutrisim-sound');
...
function initEventListeners() {
  ...
  DOM.soundBtn.addEventListener('click', () => {
    ...
    localStorage.setItem('nutrisim-sound', soundEnabled);
  ...
  DOM.themeBtn.addEventListener('click', () => {
    ...
    if (isDark) {
      ...
      document.body.classList.add('light-theme');
      DOM.themeBtn.innerHTML = '<i class="fa-solid fa-moon"></i>';
      localStorage.setItem('nutrisim-theme', 'light');
    } else {
      document.body.classList.add('dark-theme');
      document.body.classList.remove('light-theme');
      DOM.themeBtn.innerHTML = '<i class="fa-solid fa-sun"></i>';
      localStorage.setItem('nutrisim-theme', 'dark');
    }
  });
...
function endQuiz() {
  ...
  if (scorePercent >= 85) {
    ...
    message = '¡Increíble! Tienes un dominio excelente de las materias de nutrición clínica, comunitaria y gestión alimentaria.';
    playSound('success');
...
function saveScoreToLocalStorage(scoreData) {
  let scores = [];
  try {
    scores = JSON.parse(localStorage.getItem('nutrisim-scores')) || [];
  } catch (e) {
    scores = [];
  }
  scores.push(scoreData);
  localStorage.setItem('nutrisim-scores', JSON.stringify(scores));
}
